package com.mufiid.dicodingbajp.data.source.local
...
import com.mufiid.dicodingbajp.data.source.local.room.AcademyDao
...
class LocalDataSource private constructor(private val mAcademyDao: AcademyDao) {
    ...
    fun getAllModuleByCourse(courseId: String): LiveData<List<ModuleEntity>> = mAcademyDao.getModulesByCourseId(courseId)

    fun insertCourses(course: List<CourseEntity>) = mAcademyDao.insertCourses(course)
